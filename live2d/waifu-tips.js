...
live2d_settings['modelTexturesId']      = 109;           // 默认材质 ID，可在 F12 控制台找到
...
//看板娘样式设置
live2d_settings['waifuSize']            = '240x230';    // 看板娘大小，例如 '280x250', '600x535'
...
live2d_settings['waifuEdgeSide']        = 'right:68';     // 看板娘贴边方向，例如 'left:0'(靠左 0px), 'right:30'(靠右 30px)
...
live2d_settings['aboutPageUrl']         = 'https://tagbug.gitlab.io/about/';   // 关于页地址, '{URL 网址}'
...
function initModel(waifuPath, type) {
	...
	$(".waifu").css("top",$(window).height()-live2d_settings.waifuSize[1]+'px');
    if (live2d_settings.waifuEdgeSide[0] == 'left') $(".waifu").css("left",live2d_settings.waifuEdgeSide[1]+'px');
    else if (live2d_settings.waifuEdgeSide[0] == 'right') $(".waifu").css("left",($(window).width()-live2d_settings.waifuSize[0]-live2d_settings.waifuEdgeSide[1])+'px');
    ...
    $.each(result.mouseover, function (index, tips){
        $(document).on("mouseover", tips.selector, function (){
			...
			if(text.includes('{text}')){
				var tmptext = text.render({text: $(this).text()});
				if(tmptext == text.replace(/{text}/g, '')) return;
				text = tmptext;
			}
    ...
    $.each(result.seasons, function (index, tips){
        ...
        if((after.split('/')[0] <= now.getMonth()+1 && now.getMonth()+1 <= before.split('/')[0]) &&
           (after.split('/')[1] <= now.getDate() && now.getDate() <= before.split('/')[1])){
			...
			text = text.render({guoqing: now.getFullYear() - 1949});
